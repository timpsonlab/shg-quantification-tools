function CodeTMAImages(root)
    ...
    output_folder = ['Montages' filesep];
    ...
    pos_names = cellfun(@(a,b) [a num2str(b)], repmat(rows,[1 length(cols)]), repmat(cols,[length(rows) 1]), 'UniformOutput', false);

    for i=1:length(folders)

        codes_file = [root tma_folder folders{i} ' codes.csv'];
